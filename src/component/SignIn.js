...
class SignIn extends React.Component {
  ...
  onSubmit = async (event) => {
    ...
    try {
      ...
      {
        SetUser(data.data.token,{name:data.data.userInfo.name,email:data.data.userInfo.email})
  ...
  async AuthorizeClient(TwoFA) {
    ...
    localStorage.setItem(UserAuthTokenId, TwoFA === undefined ? this.state.data.is2FAEnabled : TwoFA);
    localStorage.setItem(
      UserProfileTokenId,
      JSON.stringify({
                  ...
                  <div className="card-body">
                    <div className="card-content p-2">
                      <div className="text-center">
                        <img src={LogoSmall} width="100" alt="logo icon" />
                      </div>
